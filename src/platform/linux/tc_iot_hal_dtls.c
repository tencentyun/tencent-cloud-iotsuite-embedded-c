...
int tc_iot_hal_dtls_connect(tc_iot_network_t* network, const char* host,
    ...
    ret = mbedtls_ssl_cookie_setup(&(tls_data->cookie_ctx), mbedtls_ctr_drbg_random, &(tls_data->ctr_drbg));

    if (ret != 0) {
        TC_IOT_LOG_ERROR("mbedtls_ssl_cookie_setup result %d", ret);
        return TC_IOT_CTR_DRBG_SEED_FAILED;
    }

    mbedtls_ssl_conf_dtls_cookies(&(tls_data->conf), mbedtls_ssl_cookie_write, mbedtls_ssl_cookie_check, &(tls_data->cookie_ctx));
    ...
    if ((ret =
             mbedtls_net_connect(&(tls_data->ssl_fd), network->net_context.host,
                                 port_str, MBEDTLS_NET_PROTO_UDP)) != 0) {
    ...
    mbedtls_ssl_set_bio( &(tls_data->ssl_context), &(tls_data->ssl_fd), mbedtls_net_send, mbedtls_net_recv, mbedtls_net_recv_timeout );
	mbedtls_ssl_set_timer_cb(&(tls_data->ssl_context), (void *)&(tls_data->delay_timer),
...
int tc_iot_hal_dtls_disconnect(tc_iot_network_t* network) {
    ...
    mbedtls_ssl_cookie_free(&(tls_data->cookie_ctx));
...
int tc_iot_hal_dtls_destroy(tc_iot_network_t* network) {
    ...
    if (tc_iot_hal_dtls_is_connected(network)) {
        tc_iot_hal_dtls_disconnect(network);
    }
